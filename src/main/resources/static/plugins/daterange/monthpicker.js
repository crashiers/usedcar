	...
	*/
	...
	show : function(ref) {
		...
		if(this.util.$(this._conf.container)) {
			...
			this.util.$(this._conf.container).style.left = pos.left-100 + 'px';

            // 优化左边距
            // 月历本身宽度260 + pos.left > 整体宽度document.body.offsetWidth
            if (260+pos.left >= document.body.offsetWidth ){
                this.util.$(this._conf.container).style.left = document.body.offsetWidth-260-20 + 'px';
            }
		...
		// 月历本身宽度260 + pos.left > 整体宽度document.body.offsetWidth
		if (260+pos.left >= document.body.offsetWidth ){
            div.style.left = document.body.offsetWidth-260-20 + 'px';
		}
	...
	periodCss : function(){
		...
		if(monthPicker.Params.start_year == monthPicker.Params.end_year){
			if(that.assemble.year == start.year || '' == start.year){
				...
				em = Math.max( monthPicker.Params.end_month,  monthPicker.Params.start_month);
				for(var i = sm; i<=em; i++){
                    // 注释掉下面这一行，就代表着再点开月份日历的时候，不会把在月份区间里的月份加上 选中样式。
                    //$('#gri_month'+i).addClass(that._conf.selectCss);
				}
			}
			else{
